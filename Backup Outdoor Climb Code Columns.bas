...
Sub Individual_Lists_Based_By_Columns()
    Dim ws As Worksheet
    ...
    numCols = 4 ' number of data points, Grade, Date, Name, Location
    ...
    startRow = 2 ' other variables declared everytime
    ...
    For j = 1 To 3
        For k = 1 To 7
            ...
            currentRow = startRow
            ReDim dataList(1 To totalDataRows, 1 To numCols) ' numCols only including 4 grade, date, name, location
        
            If totalDataRows >= 1 And ws.Cells(currentRow, colNum + 1).value <> "" Then

                ' Fill the dataList array
                For i = 1 To totalDataRows
                    dataList(i, 1) = ws.Cells(startRow, colNum).value ' Dates from column B
                    ...
                    currentRow = currentRow + 1
                Next i
            ' Create output string to display in MsgBox
                outputData = "Bouldering Sends " & Cells(startRow, colNum).value & ":" & vbCrLf
                For i = 1 To totalDataRows
                    outputData = outputData & dataList(i, 1) & " | " & dataList(i, 2) & " | " & dataList(i, 3) & " | " & dataList(i, 4) & vbCrLf
                Next i
        
                ' Display the output in a message box
                MsgBox outputData, vbInformation, "Bouldering Data"
            Else
                'dataList(1, 1) = "" ' Dates from column B
                'dataList(1, 2) = "" ' Names from column C
                'dataList(1, 3) = "" ' Locations from column D
                'dataList(1, 4) = ""
                MsgBox "No sends for " & Cells(startRow, colNum).value & " :(", vbExclamation, "No Data"
            End If
